...
parser.add_argument('--map', default='Town01', help='Town Map')
...
#parser.add_argument('--save_video', default=True, help='save the simulation video')
# 给绝对路径 记得改位置哦！
parser.add_argument('--save-path', default='/home/cpsgroup/trust_aware_hrc/sensor_data/', help='Synchronous mode execution')
...
def spawn_walker(world, location, trigger=0):
    blueprint_library = world.get_blueprint_library()
    walker_bp = random.choice(blueprint_library.filter('walker.pedestrian.*'))
    #transform = carla.Transform(location, rotation)
    walker = world.spawn_actor(walker_bp, location)
    return walker
...
def main(args):
    ...
    try:
        ...
        # 手动规定
        #transform_vehicle = carla.Transform(carla.Location(88, 10, 0), carla.Rotation(yaw=90))
        # 自动选择
        ...
        tm.global_percentage_speed_difference(15.0) # 10
        ...
        while True:
            ...
            ego_waypoint = world.get_map().get_waypoint(ego_vehicle.get_location()) # get the way point of ego vehicle
            ego_waypoint_get_junction = ego_waypoint.get_junction()
            ego_waypoint_id = ego_waypoint.id
            ego_road_id = ego_waypoint.road_id
            ego_lane_id = ego_waypoint.lane_id
            # spawn a walker when ego veh is on this lane
            walker_control = carla.WalkerControl()
            walker_trigger = 1
            waypoint_separation = 10
            print('ego_waypoint.next_until_lane_end: ', ego_waypoint.next_until_lane_end(waypoint_separation))
            walker_loc = ego_waypoint.next_until_lane_end(waypoint_separation)[0].transform # decide where to spawn the walker, another func:next()
            print(walker_loc)
            walker = spawn_walker(world, walker_loc, trigger=0)
            walker_control = carla.WalkerControl()
            walker_control.speed = 0.9
            walker_control.direction.y = 1
            walker_control.direction.x = 0
            walker_control.direction.z = 0
