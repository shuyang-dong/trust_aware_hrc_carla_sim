...
class World(object):
    ...
    def restart(self, args):
        ...
        while self.player is None:
            ...
            #transform = carla.Transform(carla.Location(x=90, y=1), carla.Rotation(yaw=180)) # assign the spawn point (intersection A in town01, 02)
            transform = carla.Transform(carla.Location(x=88, y=10),carla.Rotation(yaw=90))  # assign the spawn point (intersection A in town01, 02)
...
def updateSpectator(vehicle, world):
    ...
    Atment_SpringArmGhost = carla.libcarla.AttachmentType.SpringArm
...
def game_loop(args):
    ...
    # route in town 01:
    # A, B, A
    agent_destination_list_1=[[91,2], [91,3], [91,4], [91,5], [91,6],[91,16],[91,26],[91,36],[91,46], [91, 56],
                              [91, 46],[91,36],[91,26],[91,16], [91,6], [91,5], [91,4], [91,3], [91,2]]
    dest_point_id = 0

    agent_destination_list_1 = [[88, 15], [90, 36],  [91, 26], [91, 2]]
    agent_destination_list_1 = [[88, 15], [90, 36]]
    path = []
    for pair in agent_destination_list_1:
        point = carla.Location(x=pair[0], y=pair[1])
        path.append(point)
    ...
    try:
        ...
        destination = carla.Location(x=agent_destination_list_1[dest_point_id][0], y=agent_destination_list_1[dest_point_id][1])  # assign the spawn point (intersection A in town01, 02)
        ...
        while True:
            ...
            """####
            if agent.done():
                print('reach destination: ', destination)
                if args.loop:
                    spawn_points = world.map.get_spawn_points()
                    ...
                    if dest_point_id!=len(agent_destination_list_1)-1:
                        dest_point_id += 1
                        destination = carla.Location(x=agent_destination_list_1[dest_point_id][0], y=agent_destination_list_1[dest_point_id][1])
                        print('set destination: ', destination)
                        agent.set_destination(destination)
                    else:
