...
def syn_full_plan(prod_mdp, gamma, alpha=1):
    ...
    for l, S_fi in enumerate(prod_mdp.Sf):
        ...
        for k, MEC in enumerate(S_fi):
            plan_prefix, prefix_cost, prefix_risk, y_in_sf, Sr, Sd = syn_plan_prefix(
                prod_mdp, MEC, gamma)
            print("Best plan prefix obtained, obj_value: %s, risk %s" %
        ...
        if plan:
            best_k_plan = max(plan, key=lambda p: p[0][1]- 10*p[1][1])
    ...
    if Plan:
        ...
        print("=========================")
        best_all_plan = max(Plan, key=lambda p: p[0][1]- 10*p[1][1])
        f_best_plan_prefix = open('to_matlab/data/best_plan_prefix.dat','w')
        f_best_plan_suffix = open('to_matlab/data/best_plan_suffix.dat','w')
        print('Best plan prefix obtained for %s states in Sr' %
              str(len(best_all_plan[0][0])))
        best_plan_prefix = best_all_plan[0][0]
        with open('pickle_for_prism/best_plan_prefix.pkl', 'wb') as pickle_file:
            pickle.dump(best_plan_prefix, pickle_file)
        for state, act_prob in best_plan_prefix.items():
            f_best_plan_prefix.write('%s, %s\n' %(state, act_prob))
        print('obj_value: %s; risk: %s ' %
              (best_all_plan[0][1], best_all_plan[0][2]))
        print('Best plan suffix obtained for %s states in Sf' %
              str(len(best_all_plan[1][0])))
        best_plan_suffix = best_all_plan[1][0]
        with open('pickle_for_prism/best_plan_suffix.pkl', 'wb') as pickle_file:
            pickle.dump(best_plan_suffix, pickle_file)
        for state, act_prob in best_plan_suffix.items():
            f_best_plan_suffix.write('%s, %s\n' %(state, act_prob))
        print('cost: %s; risk: %s ' %
              (best_all_plan[1][1], best_all_plan[1][2]))
        print('Total cost:%s' %
               (best_all_plan[0][1] -10*best_all_plan[1][1]))
    ...
    for init_node in prod_mdp.graph['initial']:
        ...
        if True:
                ...
                for t in prod_mdp.successors(s):
                    if t in Sd:
                        ...
                        for u in prop.keys():
                            pe = prop[u][0]
                            y_to_sf += Y[(s, u)]*pe
            prefix_solver.Maximize(50*y_to_sf-obj)
            ...
            for s in Sr:
                ...
                plan_prefix[s] = [U, P]
                f_plan_prefix_dict.write('%s, %s, %s\n' %(s, U, P)) 
            print("----Prefix plan generated")
            obj_value = prefix_solver.Objective().Value()
            print("----Prefix objective value computed, cost: %.2f" % obj_value)
            ...
            return plan_prefix, obj_value, risk, y_in_sf, Sr, Sd
        # except:
    ...
    for init_node in prod_mdp.graph['initial']:
        ...
        if True:
            ...
            for s in Sn:
                ...
                plan_suffix[s] = [U, P]
                f_plan_suffix_dict.write('%s, %s, %s\n' %(s, U, P)) 
