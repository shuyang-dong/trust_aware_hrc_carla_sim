...
public:
  ...
  void convert(NBA_t& nba,
    ...
    if (_options.dba_direct && nba.isDeterministic()) {
      if (can_stutter()) {
	convert_dba2dra_stuttered(nba, dra_result, limit);
      } else {
	convert_dba2dra(nba, dra_result);
      }
      dra_result.setComment("DBA2DRA[NBA="+std::to_string(nba.size())+"]");
      return;
    }
  ...
  /**
   * Convert the deterministic NBA to a DRA
   * @param nba the NBA
   * @param dra_result the result DRA
   * @param limit limit for the size of the DRA
   */
  template < typename NBA_t,
	     typename DRA_t >
  void convert_dba2dra(NBA_t& nba, DRA_t& dra_result, std::size_t limit=0) {
    typedef DBA2DRAAlgorithm<NBA_t> algo_t;
    algo_t algo(nba);

    NBA2DA<algo_t, DRA_t>
      nba2da(_detailed_states);

    nba2da.convert(algo, dra_result, limit);
  }


  /**
   * Convert the deterministic NBA to a DRA, using stuttering
   * @param nba the NBA
   * @param dra_result the result DRA
   * @param limit limit for the size of the DRA
   */
  template < typename NBA_t,
	     typename DRA_t >
  void convert_dba2dra_stuttered(NBA_t& nba, DRA_t& dra_result, std::size_t limit=0) {
    typedef DBA2DRAAlgorithm<NBA_t> algo_t;
    algo_t algo(nba);

    StutteredNBA2DA<algo_t, DRA_t>
      nba2dra_stuttered(_detailed_states, _stutter_information);

    nba2dra_stuttered.convert(algo, dra_result, limit);
  }
