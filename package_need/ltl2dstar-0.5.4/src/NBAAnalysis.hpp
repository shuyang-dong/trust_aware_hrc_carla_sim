...
public:
  ...
  /** Remove surely rejecting states. Modifies the NBA! */
  void removeRejecting() {
    BitSet rejecting = calculateRejectingStates();

    if (rejecting.isEmpty())
      return;

    // remove rejecting states
    _nba.remove(rejecting);

    // as the NBA has changed, remove all analyses,
    // will be recomputed
    clearAnalysis();
  }
...
private:
  ...
  void clearAnalysis() {
    _sccs.reset();
    _allSuccAccepting.reset();
    _accepting_true_loops.reset();
    _reachability.reset();
  }
  ...
  /**
   * Calculate rejecting states, i.e., those that can not reach a SCC
   * containing an accepting state.
   */
  BitSet calculateRejectingStates() {
    BitSet states = getSCCs().statesCanReach(_nba.getFinalStates());
    // rejecting = !states
    states.flip(0, _nba.size()>0 ? _nba.size()-1 : 0);
    return states;
  }
