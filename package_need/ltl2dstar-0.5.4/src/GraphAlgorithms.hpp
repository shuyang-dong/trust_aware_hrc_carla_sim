...
public:
  ...
  std::size_t state2scc(std::size_t state) const {
    return _state_to_scc[state];
  }

  /** Get the set of SCCs covering the given set of states */
  BitSet states2sccs(const BitSet& states) const {
    BitSet sccs;
    for (std::size_t state : states) {
      sccs.set(state2scc(state));
    }
    return sccs;
  }
  ...
  bool isReachable(std::size_t scc_from,
		   std::size_t scc_to) const {
    return _reachability[scc_from].get(scc_to);
  }

  /**
   * Return true if any of the target SCCs is reachable from
   * the scc_from SCC. If target_sccs contains scc_from, return
   * true as well.
   */
  bool isReachable(std::size_t scc_from, BitSet target_sccs) const {
    if (target_sccs.get(scc_from))
      return true;

    for (std::size_t target_scc : target_sccs) {
      if (isReachable(scc_from, target_scc)) {
	return true;
      }
    }
    return false;
  ...
  /** Return the set of states that can reach the target set of states */
  BitSet statesCanReach(BitSet target) {
    // the target SCCs
    BitSet target_sccs = states2sccs(target);

    // the SCCs that can reach a target SCC
    BitSet sccs_can_reach;
    for (std::size_t scc_index = 0, n = countSCCs(); scc_index < n; scc_index++) {
      if (isReachable(scc_index, target_sccs)) {
	sccs_can_reach.set(scc_index);
      }
    }

    // lift back to states
    BitSet result;
    for (std::size_t scc_index : sccs_can_reach) {
      result.Or(_sccs[scc_index]);
    }
