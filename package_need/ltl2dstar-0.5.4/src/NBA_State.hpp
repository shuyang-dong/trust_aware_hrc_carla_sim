...
class NBA_State : public Indexable<NBA_State<Label,EdgeContainer> > {
  ...
  */
  ...
  /** A wrapper around a pair of edge iterators (for range-based loops). */
  struct edge_iterators {
    NBA_State& state;

    edge_iterators(NBA_State& state) : state(state) {}
    typename edge_container_type::EdgeIterator begin() {return state.edges_begin();}
    typename edge_container_type::EdgeIterator end() {return state.edges_end();}
  };

  /** Return wrapper around edge iterators (for range-based loops). */
  edge_iterators edges() {return edge_iterators(*this);}
