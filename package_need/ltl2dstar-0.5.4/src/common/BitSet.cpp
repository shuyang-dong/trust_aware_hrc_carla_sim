...
/** Iterator for the set bits (begin) */
_BITSET_INLINE BitSetIterator BitSet::begin() const {
  return BitSetIterator(*this);
}

/** Iterator for the set bits (end) */
_BITSET_INLINE BitSetIterator BitSet::end() const {
  return BitSetIterator::end(*this);
}

/**
 * Map values according to a map.
 * The map is provided by a vector, containing the target locations
 * for a given source index. The target location has to be less-than
 * or equal to the source location!
 *
 * Only set bits are moved.
 */
_BITSET_INLINE void BitSet::map(std::vector<std::size_t>& map) {
  for (std::size_t from=0, n=map.size(); from < n; from++) {
    std::size_t to=map[from];
    if (from == to)
      continue;
    if (to > from)
      THROW_EXCEPTION(IllegalArgumentException, "target location > source location");
